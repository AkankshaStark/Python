...
class DataFilterGUI:
    ...
    def display_column_values(self, column_name):
        ...
        if pd.api.types.is_numeric_dtype(self.tab_df[column_name]):
            ...
            min_var = DoubleVar(value=min_value)
            max_var = DoubleVar(value=max_value)

            min_label = Label(self.display_frame, text="Min:")
            min_label.pack()
            min_entry = Entry(self.display_frame, textvariable=min_var)
            min_entry.pack()

            max_label = Label(self.display_frame, text="Max:")
            max_label.pack()
            max_entry = Entry(self.display_frame, textvariable=max_var)
            max_entry.pack()
            ...
            canvas = Canvas(self.display_frame, highlightthickness=0)
    ...
    def display_columns_selection(self, filtered_df):
        ...
        label = Label(self.display_frame, text="Select Columns to Display in Filtered Output", font=("Arial", 14),
                      fg="brown")
        label.pack(pady=10)

        # Create a scrollable frame
        canvas = Canvas(self.display_frame)
        scrollbar = Scrollbar(self.display_frame, orient="vertical", command=canvas.yview)
        ...
        confirm_button = Button(
            ...
            command=lambda: self.apply_column_selection(filtered_df, column_vars),
            bg="green",
            fg="brown"
        )
        confirm_button.pack(pady=10)

    def apply_column_selection(self, filtered_df, column_vars):
        """
        ...
        self.display_results(filtered_data_display)
    ...
    def display_results(self, filtered_df):
        try:
            ...
            results_frame = Frame(self.display_frame)
    ...
    def show_graph(self, filtered_df):
        ...
        try:
            ...
            chart_type_dropdown = OptionMenu(self.display_frame, chart_type_var, *chart_type_options)
            ...
            x_dropdown = OptionMenu(self.display_frame, x_var, *filtered_df.columns)
            ...
            y_dropdown = OptionMenu(self.display_frame, y_var, *y_options)
...
class InputGUI:
    ...
    def create_file_upload_section(self, db_name, section_number):
        ...
        inner_frame = Frame(border_frame, bg='#FFFFFF')
